...
class GraphAssimilator(nn.Module):
    def __init__(
        self,
        in_dim_obs,
        pos_dim,
        ...
        self.kernel_mlp = nn.Sequential(
            nn.Linear(3 * pos_dim, latent_dim),
            nn.ReLU(),
        ...
        # ---- Output: mean + logvar ----
        decoder_in = latent_dim * 2 if self.use_bg else latent_dim
    ...
    def forward(
        ...
        # this is a fully connected relative position encoding
        ...
        if self.use_bg:
            ...
            h_query = torch.cat([h_query, h_bg], dim=-1) # (d*2,)

        # ---- Decode ----
        out = self.decoder(h_query)
