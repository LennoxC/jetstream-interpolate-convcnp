...
class AMDARInterface:
    ...
    def fetch_partition(self, year, month, day, lat, lon):
        ...
        partition_path = f"{self.save_path}year={year}/month={month}/day={day}/coarse_{LATITUDE}={int(np.floor(lat))}/coarse_{LONGITUDE}={int(np.floor(lon))}"
        df = dd.read_parquet(partition_path).compute()
        return df
        ...
        for date in pd.date_range(date_start, date_end):
            ...
            for lat in lat_partitions:
                for lon in lon_partitions:
                    paths.append(f"{self.save_path}year={date.year}/month={date.month}/day={date.day}/coarse_{LATITUDE}={lat}/coarse_{LONGITUDE}={lon}/part.*.parquet")
        
        # read all the data in a dask dataframe
        df = dd.read_parquet(paths)

        if (len(df) == 0):
            raise ValueError(f"No data found for the given lat/lon box and time window. Paths searched: {paths}")
